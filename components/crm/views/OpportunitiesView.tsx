...
import { ObjectHeader, accountName, currency, slug, translateValue } from "../workspace/helpers";
...
function OpportunityKanban({ data, actions, permissions }: { data: CrmData; actions: Actions; permissions: CrmPermissions }) {
  ...
  return (
    <section className="page-stack" data-testid="page-opportunity-kanban">
      <ObjectHeader title={tKanban("title")} subtitle={tKanban("subtitle")}>
        {permissions.canWrite ? <button className="button primary" data-testid="btn-new-opportunity" onClick={() => actions.openCreate("opportunity")}>{tHome("newOpportunity")}</button> : null}
      </ObjectHeader>
          ...
          return (
            <section className="kanban-column" key={stage} data-testid={`kanban-column-${slug(stage)}`}>
              <div className="kanban-column-header">
                <h2>{translateValue(stage, locale)}</h2>
